...
class BayesianCF():
    ...
    def predict(self, trainX, trainT, X_pred):
        ...
        for i in range(self.N):
            Ssum = np.add(Ssum, phi(trainX[i], self.M).dot(
                phi(X_pred, self.M).transpose()))
    ...
    def autoadjust(self):
        ...
        for self.M in range(20):
            ...
            for t in range(len(self.x) - 24 * 7):
                y_fit[t] = self.predict(
                    self.x[t:t+24*7], self.y[t:t+24*7], self.x[t+24*7-1]+60)
